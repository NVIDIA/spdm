...
class SpdmTool
{
  ...
  private:
    ...
    auto interpretResponse(std::vector<uint8_t>& buf, T& packet, Targs... fargs)
        -> spdmcpp::RetStat;
    ...
    auto parseCertChain(std::vector<uint8_t>& vec, std::string& out)
        -> spdmcpp::RetStat;
...
//! Interpret response
template <typename T, typename... Targs>
auto SpdmTool::interpretResponse(std::vector<uint8_t>& buf, T& packet,
                                 Targs... fargs) -> spdmcpp::RetStat
{
    using namespace spdmcpp;
    TransportClass::LayerState lay;
    if (transport)
