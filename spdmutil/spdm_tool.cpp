...
auto SpdmTool::parseArgs(int argc, char** argv) -> int
{
    ...
    {
        std::cerr << "--eid or --enumerate should be choosen at the same time"
                  << std::endl;
        return EXIT_FAILURE;
    }
    if (!needEnumEps && (m_eid == cmdCliInvalid))
    ...
    if (!jsonFilename.empty())
    {
        ...
        if (!jsonFileStream)
        {
            std::cerr << "Unable to create json file: " << jsonFilename
                      << std::endl;
            return EXIT_FAILURE;
        }
    }
    ...
    {
        auto it = std::find_if(cmdList.rbegin(), cmdList.rend(),
        ...
        const auto idx = std::min<size_t>(
            std::distance(it, cmdList.rend()) - 1U, cmdList.size());
...
template <>
spdmcpp::RetStat
    SpdmTool::handleRecv<PacketVersionResponseVar>(std::vector<uint8_t>& buf)
{
...
template <>
spdmcpp::RetStat
    SpdmTool::handleRecv<PacketCapabilitiesResponse>(std::vector<uint8_t>& buf)
{
    PacketCapabilitiesResponse resp;
    auto rs = interpretResponse(buf, resp);
    if (isError(rs))
    {
        jsonGen["GetCapabilities"] = {{"ResponseCode", get_cstr(rs)}};
        return rs;
    }
...
template <>
spdmcpp::RetStat
    SpdmTool::handleRecv<PacketAlgorithmsResponseVar>(std::vector<uint8_t>& buf)
{
    ...
    if (isError(rs))
    {
        jsonGen["NegotiateAlgorithms"] = {{"ResponseCode", get_cstr(rs)}};
        return rs;
    }
    algoResp = resp;
...
template <>
spdmcpp::RetStat SpdmTool::handleRecv<PacketCertificateResponseVar>(
...
template <>
spdmcpp::RetStat
    SpdmTool::handleRecv<PacketDigestsResponseVar>(std::vector<uint8_t>& buf)
{
    PacketDigestsResponseVar resp;
    auto rs = interpretResponse(buf, resp, packetDecodeInfo);
    if (isError(rs))
    {
        jsonGen["GetDigest"] = {{"ResponseCode", get_cstr(rs)}};
        return rs;
    }
    std::vector<std::vector<int>> digest;
    digest.reserve(resp.Digests.size());
    std::transform(
        resp.Digests.begin(), resp.Digests.end(), std::back_inserter(digest),
...
template <>
spdmcpp::RetStat SpdmTool::handleRecv<PacketMeasurementsResponseVar>(
    std::vector<uint8_t>& buf)
{
    ...
    if (isError(rs))
    {
        jsonGen["GetMeasurement"] = {{"ResponseCode", get_cstr(rs)}};
        return rs;
    }
...
auto SpdmTool::runComm() -> bool
{
    if (std::all_of(
            ...
            [](const std::optional<cmdv>& opt) { return !opt.has_value(); }))
    {
        return true;
    }
    try
...
auto SpdmTool::parseResp(std::vector<uint8_t>& buf) -> spdmcpp::RetStat
{
    ...
    {
        if (code != PacketCertificateResponseVar::requestResponseCode &&
            wholeCert)
        {
...
auto SpdmTool::parseCertChain(std::vector<uint8_t>& vec, std::string& out)
    -> spdmcpp::RetStat
{
