...
// Cover get_cstr for MessageVersionEnum (gcov report)
TEST(Enum, GetCstrMessageVersionEnum)
{
    const char* str = get_cstr(MessageVersionEnum::SPDM_1_0);
    EXPECT_NE(str, nullptr);
    EXPECT_NE(std::string(str).find("1_0"), std::string::npos);
    str = get_cstr(MessageVersionEnum::SPDM_1_1);
    EXPECT_NE(str, nullptr);
    str = get_cstr(MessageVersionEnum::SPDM_1_2);
    EXPECT_NE(str, nullptr);
}

// Cover get_cstr for RequestResponseEnum (gcov report)
TEST(Enum, GetCstrRequestResponseEnum)
{
    const char* str = get_cstr(RequestResponseEnum::REQUEST_GET_VERSION);
    EXPECT_NE(str, nullptr);
    str = get_cstr(RequestResponseEnum::RESPONSE_VERSION);
    EXPECT_NE(str, nullptr);
}

// Cover get_cstr for AlgTypeEnum (gcov report)
TEST(Enum, GetCstrAlgTypeEnum)
{
    const char* str = get_cstr(AlgTypeEnum::DHE);
    EXPECT_NE(str, nullptr);
    str = get_cstr(AlgTypeEnum::ReqBaseAsymAlg);
    EXPECT_NE(str, nullptr);
}
