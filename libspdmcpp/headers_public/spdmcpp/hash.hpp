...
namespace spdmcpp
{
	...
	public:
		...
		{
			mbedtls_md_init(&Ctx);
			int ret = mbedtls_md_setup(&Ctx, Ctx.md_info, 0);//TODO md_info may be considered private?!
		...
		void update(const std::vector<uint8_t>& buf, size_t off = 0, size_t len = std::numeric_limits<size_t>::max())
		{
			assert(off < buf.size());
