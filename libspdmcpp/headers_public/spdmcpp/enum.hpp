...
namespace spdmcpp
{
	#define ENUM_START(T,UT)		enum class T : UT {
	...
	#define ENUM_START(T,UT)		\
		inline const char* get_cstr(T e)	{	\
			switch(e) {
	#define ENUM_VALUE(T,N,V)	case T::N: return #T "::" #N;
	#define ENUM_END()		\
			}		\
			return "UNKNOWN";		\
		}
	...
	constexpr inline bool is_request(RequestResponseEnum code) {
		return code >= RequestResponseEnum::REQUEST_GET_DIGESTS && code <= RequestResponseEnum::REQUEST_END_SESSION;
	}
	constexpr inline bool is_response(RequestResponseEnum code) {
		return code >= RequestResponseEnum::RESPONSE_DIGESTS && code <= RequestResponseEnum::RESPONSE_END_SESSION_ACK;
	}
