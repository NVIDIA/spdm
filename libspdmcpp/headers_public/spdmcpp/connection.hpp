...
namespace spdmcpp
{
	...
	public:
		...
		MessageVersionEnum getMessageVersion() const { return MessageVersion; }
		std::vector<uint8_t>& getResponseBufferRef() { return ResponseBuffer; }
		
		LogClass& getLog() { return Log; }
	...
	protected:
		...
		struct SlotClass
		{
			...
			mbedtls_x509_crt* GetRootCert() const { assert(MCertificates.size() >= 2); return MCertificates[0]; }
...
// 		template<typename T> RetStat interpret_response(T& packet);
		...
		uint8_t GotInfo = 0;
		static_assert(sizeof(GotInfo) * 8 >= static_cast<std::underlying_type_t<ConnectionInfoEnum>>(ConnectionInfoEnum::NUM));
		
		void MarkInfo(ConnectionInfoEnum info) { GotInfo |= 1 << static_cast<std::underlying_type_t<ConnectionInfoEnum>>(info); }
		bool HasInfo(ConnectionInfoEnum info) { return !!(GotInfo & (1 << static_cast<std::underlying_type_t<ConnectionInfoEnum>>(info))); }
