...
namespace spdmcpp
{
	...
	{
		OK										= 0,
		WARNING_BUFFER_TOO_BIG					= 1,//TODO some of these may be better as flags?! but then we must use helper functions or better make it a simple class?
		ERROR_UNKNOWN							= -1,
	...
	//TODO do we want different layers/types of codes to avoid checking for errors that don't make sense for a function and other such issues?! we could also have helpers to change from one to another in cases where it's sensible
