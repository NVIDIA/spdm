...
namespace spdmcpp
{
	...
	RetStat ConnectionClass::try_get_version()
	{
		...
		auto rs = send_request_setup_response(spdm_request, packet_get_version_response_var(), BufEnum::A);
		SPDMCPP_LOG_TRACE_RS(Log, rs);
		return rs;
	}
	template<>
	...
	RetStat ConnectionClass::choose_version()
	{
		...
		std::vector<MessageVersionEnum> vers;//TODO is using just the enum fine or do we need more detailed info?!
...
// 			request.Flags = RequesterCapabilitiesFlags::CERT_CAP | RequesterCapabilitiesFlags::CHAL_CAP | RequesterCapabilitiesFlags::ENCRYPT_CAP | RequesterCapabilitiesFlags::MAC_CAP;
		request.Flags = RequesterCapabilitiesFlags::CHAL_CAP;
	/*	request.Flags |= RequesterCapabilitiesFlags::ENCRYPT_CAP | RequesterCapabilitiesFlags::MAC_CAP;
	//	request.Flags |= RequesterCapabilitiesFlags::MUT_AUTH_CAP;
		...
		*/
		auto rs = send_request_setup_response(request, packet_get_capabilities_response(), BufEnum::A);
		SPDMCPP_LOG_TRACE_RS(Log, rs);
		return rs;
	}
	template<>
...
// 			request.Min.BaseAsymAlgo = BaseAsymAlgoFlags::TPM_ALG_RSASSA_2048 | BaseAsymAlgoFlags::TPM_ALG_RSAPSS_2048;
		request.Min.BaseAsymAlgo = BaseAsymAlgoFlags::TPM_ALG_ECDSA_ECC_NIST_P256 | BaseAsymAlgoFlags::TPM_ALG_ECDSA_ECC_NIST_P384;
		request.Min.BaseHashAlgo = BaseHashAlgoFlags::TPM_ALG_SHA_256 | BaseHashAlgoFlags::TPM_ALG_SHA_384 | BaseHashAlgoFlags::TPM_ALG_SHA_512;
// 			request.Flags = RequesterCapabilitiesFlags::CERT_CAP | RequesterCapabilitiesFlags::CHAL_CAP;
		
		request.PacketReqAlgVector.push_back(PacketReqAlgStruct::buildSupported2(AlgTypeEnum::DHE, 0x1b, 0x00));
		request.PacketReqAlgVector.push_back(PacketReqAlgStruct::buildSupported2(AlgTypeEnum::AEADCipherSuite, 0x06, 0x00));
		request.PacketReqAlgVector.push_back(PacketReqAlgStruct::buildSupported2(AlgTypeEnum::ReqBaseAsymAlg, 0x0F, 0x00));
		request.PacketReqAlgVector.push_back(PacketReqAlgStruct::buildSupported2(AlgTypeEnum::KeySchedule, 0x01, 0x00));
		request.finalize();
		
		auto rs = send_request_setup_response(request, packet_algorithms_response_var(), BufEnum::A);
		SPDMCPP_LOG_TRACE_RS(Log, rs);
		return rs;
	}
	template<>
	...
	//	PacketDecodeInfo.MeasurementHashSize = mbedtls_md_info_from_type(to_mbedtls(resp.Min.MeasurementHashAlgo)).size;
	//	PacketDecodeInfo.BaseHashSize = mbedtls_md_info_from_type(to_mbedtls(resp.Min.BaseHashAlgo)).size;
		PacketDecodeInfo.MeasurementHashSize = get_hash_size(resp.Min.MeasurementHashAlgo);
	...
	RetStat ConnectionClass::try_get_digest()
	{
		...
		auto rs = send_request_setup_response(request, packet_digests_response_var(), BufEnum::B);
		SPDMCPP_LOG_TRACE_RS(Log, rs);
		return rs;
	}
	template<>
...
// 		request.Length = 0x400;
		...
		auto rs = send_request_setup_response(request, packet_certificate_response_var(), BufEnum::B);
		SPDMCPP_LOG_TRACE_RS(Log, rs);
		return rs;
	}
	template<>
	...
	//	if (request.Offset != cert.size()) {//TODO
		{
			...
			std::copy(resp.CertificateVector.begin(), resp.CertificateVector.end(), cert.begin() + off);//TODO @Timon optimize?!
		}
...
// 			cert[12] = 'X';	//TODO TEST
		...
		/*	{
			...
			}*/
			{
				...
				int ret = mbedtls_md(mbedtls_md_info_from_type(to_mbedtls(Algorithms.Min.BaseHashAlgo)), cert.data(), cert.size(), hash.data());
				assert(ret == 0);
			...
			packet_certificate_chain cert_chain;//TODO create a standard _var packet and just parse from cert_chain? seems like it fits surprisingly nicely!
			size_t off = 0;
			...
			//	int ret = mbedtls_x509_crt_parse_der(c, &cert[off], cert.size() - off);
				...
				size_t asn1_len = 0;
				{
					...
					uint8_t* p = s;
					ret = mbedtls_asn1_get_tag(&p, cert.data() + cert.size(), &asn1_len, MBEDTLS_ASN1_CONSTRUCTED | MBEDTLS_ASN1_SEQUENCE);
					assert(ret == 0);
					asn1_len += (p - s);
				}
// 				cert[off + 12] = 'X';	//TODO TEST
				if (slot.MCertificates.size() == 1) {
					std::vector<uint8_t> hash;
					hash.resize(get_hash_size(Algorithms.Min.BaseHashAlgo));
					int ret = mbedtls_md(mbedtls_md_info_from_type(to_mbedtls(Algorithms.Min.BaseHashAlgo)), cert.data() + off, asn1_len, hash.data());
					assert(ret == 0);
					Log.iprint("computed root certificate hash = ");
					Log.println(hash.data(), hash.size());
				if (!std::equal(hash.begin(), hash.end(), root_cert_hash.begin())) {
		...
		#if 1
			for (size_t i = 1; i < slot.MCertificates.size(); ++i) {//TODO verify/fix
				slot.MCertificates[i]->next = slot.MCertificates[i - 1];
				uint32_t rflags = 0;
// 				int ret = mbedtls_x509_crt_verify(slot.GetLeafCert(), slot.GetRootCert(), nullptr, "intel test ECP256 responder cert", &rflags, nullptr, nullptr);
// 				int ret = mbedtls_x509_crt_verify(slot.GetLeafCert(), slot.GetRootCert(), nullptr, nullptr, &rflags, nullptr, nullptr);
				int ret = mbedtls_x509_crt_verify(slot.MCertificates[i - 1], slot.MCertificates[i], nullptr, nullptr, &rflags, nullptr, nullptr);
				Log.iprint("mbedtls_x509_crt_verify ret = ");
				Log.println(ret);
				if (ret) {
					std::string info;
					info.resize(4096);
					ret = mbedtls_x509_crt_verify_info(info.data(), info.size(), "", rflags);
					assert(ret >= 0);
					info.resize(ret);
					Log.print(info);
					assert(false);
...
// 		request.Header.Param2 = PacketDecodeInfo.ChallengeParam2 = 1;
		...
		auto rs = send_request_setup_response(request, packet_challenge_auth_response_var(), BufEnum::C);
		SPDMCPP_LOG_TRACE_RS(Log, rs);
		assert(rs == RetStat::OK);
		return rs;
	}
...
// 		Context->IO->setup_timeout(1000 * 1000);
	...
	EventRetStat ConnectionClass::handle_recv()
	{
		...
		RequestResponseEnum code;
		{
			...
			code = packet_message_header_get_requestresponsecode(ResponseBuffer.data() + ResponseBufferSPDMOffset);
		}
		assert(code == WaitingForResponse || code == RequestResponseEnum::RESPONSE_ERROR);
		WaitingForResponse = RequestResponseEnum::INVALID;
		
		RetStat rs = RetStat::ERROR_UNKNOWN;
		switch (code) {
#define DTYPE(type)	case type::RequestResponseCode:	rs = handle_recv<type>();	break;
