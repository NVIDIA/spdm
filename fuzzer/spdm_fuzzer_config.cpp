...
WrapperConfig::Threshold WrapperConfig::proc2thr(double proc)
{
    ...
    {
        result.value = (uint8_t)((double)(std::numeric_limits<uint8_t>::max()) *
                                 (1.0 - proc / 100.0));
    }
    return result;
}
...
double WrapperConfig::thr2proc(WrapperConfig::Threshold thr)
{
    ...
    {
        result = 100.0 *
                 (double)(std::numeric_limits<uint8_t>::max() - thr.value) /
...
std::ostream& operator<<(std::ostream& out, const WrapperConfig& config)
{
    ...
    out << "Altering header probability:            "
        << WrapperConfig::thr2proc(config.alterHeaderThr.all) << "%"
        << std::endl;
    // out<<std::setprecision(0);
    if (config.source == WrapperConfig::Source::Generator)
    {
        out << "\tVersion alter probability:      "
            << WrapperConfig::thr2proc(config.alterHeaderThr.version) << "%"
            << std::endl;
        out << "\tMessage Type alter probability: "
            << WrapperConfig::thr2proc(config.alterHeaderThr.messageType) << "%"
            << std::endl;
        out << "\tParam 1/2 alter probability:    "
            << WrapperConfig::thr2proc(config.alterHeaderThr.param) << "%"
            << std::endl;
    }
    out << "Altering payloads' data probability:    "
        << WrapperConfig::thr2proc(config.alterDataThr) << "%" << std::endl;
