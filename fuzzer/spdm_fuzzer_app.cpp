...
bool SpdmWrapperApp::run(BaseAsymAlgoFlags asymAlgo, BaseHashAlgoFlags hashAlgo)
{
    ...
    FixtureIOClass ioResponder(
        config.enableLogTrace ? "commlog_responder.txt" : "", queue1, queue2);
    auto ioRequester = std::make_shared<FixtureIOClass>(
        config.enableLogTrace ? "commlog_requester.txt" : "", queue2, queue1);
    ...
    FuzzingResponder responder(ioResponder, trans, config, predefinedResponses,
                               asymAlgo, hashAlgo, str);
    ...
    for (;;)
    {
        ...
        else
        {
            ...
            if (rs == RetStat::OK &&
                requester.getExpectedResponse() == RequestResponseEnum::INVALID)
            {
                result = true;
                break;
                // All OK End.
...
void SpdmWrapperApp::setupCli(int argc, char** argv)
{
    ...
    bool useGrammar{};
    app.add_flag("-e, --exitAfterFirstFuzzing", config.exitAfterFirstFuzzing,
                 "Exit after first fuzzing");
    app.add_flag("-g, --grammar", useGrammar, "Use grammar in fuzz generator");
    app.add_flag("--enableLogTrace", config.enableLogTrace,
    ...
    app.add_option(
           "--fResponsesAll", doFuseResponseMessages.all,
    ...
    app.add_option("--fRespMeasurements", doFuseResponseMessages.measurements,
        ...
        ->check(CLI::Range(0.0, 100.0));
    app.add_option(
           "--fRespMeasurementsNonce", respMearurement.nonce,
           "Prob. of altering resp. msg. Measurements Nonce vector (0-100)")
        ->check(CLI::Range(0.0, 100.0));
    app.add_option(
           "--fRespMeasurementsBlock", respMearurement.measurementBlock,
           "Prob. of altering resp. msg. Measurements Block with data (0-100)")
        ->check(CLI::Range(0.0, 100.0));
    app.add_option(
           "--fRespMeasurementsBlockLen", respMearurement.measurementBlockLen,
           "Prob. of altering resp. msg. Measurements Length of Block with data (0-100)")
        ->check(CLI::Range(0.0, 100.0));
    app.add_option(
           "--fRespMeasurementsBlockVal", respMearurement.measurementBlockVal,
    ...
    app.add_option("--fRespMeasurementsOpaque", respMearurement.opaqueData,
        ...
        ->check(CLI::Range(0.0, 100.0));
    app.add_option(
           "--fRespMeasurementsOpaqueLen", respMearurement.opaqueDataLen,
           "Prob. of altering resp. msg. Measurements Length of Opaque with data (0-100)")
        ->check(CLI::Range(0.0, 100.0));
    app.add_option(
           "--fRespMeasurementsOpaqueVal", respMearurement.opaqueDataVal,
    ...
    app.add_option("--fRespMeasurementsSignature", respMearurement.signature,
        ...
        ->check(CLI::Range(0.0, 100.0));
    app.add_option(
           "--fRespMeasurementsSignatureLen", respMearurement.signatureLen,
           "Prob. of altering resp. msg. Measurements signature's length (0-100)")
        ->check(CLI::Range(0.0, 100.0));
    app.add_option(
           "--fRespMeasurementsSignatureVal", respMearurement.signatureVal,
    ...
    app.add_option("--alterHeader", alterHeaderProb.all,
                   "Probability of altering header (0-100)")
        ->check(CLI::Range(0.0, 100.0));
    app.add_option("--alterHeaderVersion", alterHeaderProb.version,
                   "Probability of altering version in header (0-100)")
        ->check(CLI::Range(0.0, 100.0));
    app.add_option("--alterHeaderType", alterHeaderProb.messageType,
                   "Probability of altering message type in header (0-100)")
        ->check(CLI::Range(0.0, 100.0));
    app.add_option("--alterHeaderParam", alterHeaderProb.param,
                   "Probability of altering param1 or param2 in header (0-100)")
        ->check(CLI::Range(0.0, 100.0));

    double alterDataProb = NAN;
    app.add_option("--alterPayloadData", alterDataProb,
                   "Probability of altering payloads' data (0-100)")
        ->check(CLI::Range(0.0, 100.0));

    std::string logFilePath;
    app.add_option(
        "--readLogFile", logFilePath,
        "Read log file with responses in order to reuse those responses");

    std::string responsesFilePath;
    app.add_option(
        "--readResponseFile", responsesFilePath,
        "Read file with responses (in hex format) in order to reuse those responses");

    app.add_option("--maxMsgExchange", config.maxIter,
                   "Maximum number of messages sent by mocked responder")
        ->check(CLI::Range(0, 100));

    app.add_option("-f, --instructionFile", config.instructionFilename,
                   "file with instruction that replaces input stream");
    ...
    if (!config.instructionFilename.empty())
    {
        std::cout << "Fuzzer reads instructions from file: "
                  << config.instructionFilename << std::endl;
    }
    ...
    {
        config.source = (useGrammar) ? WrapperConfig::Source::Generator
                                     : WrapperConfig::Source::RandomStream;
    }

    if (config.source == WrapperConfig::Source::File)
    {
        std::cout << "Predefined messages ("
                  << predefinedResponses.getAllResponses().size() << ")"
                  << std::endl;
        for (auto& [key, value] : predefinedResponses.getAllResponses())
        {
            std::cout << "Type: " << std::hex << int(key) << std::dec
                      << " Value vec len: " << value.size() << std::endl;
        }
    }

    if (doFuseResponseMessages.all > 0)
    {
        config.fuseThrRespMessages.version =
            WrapperConfig::proc2thr(doFuseResponseMessages.all);
        config.fuseThrRespMessages.capability =
            WrapperConfig::proc2thr(doFuseResponseMessages.all);
        config.fuseThrRespMessages.algorithms =
            WrapperConfig::proc2thr(doFuseResponseMessages.all);
        config.fuseThrRespMessages.digests =
            WrapperConfig::proc2thr(doFuseResponseMessages.all);
    ...
    std::cout << "doFuseResponseMessages.all        = "
              << doFuseResponseMessages.all << std::endl;
    std::cout << "doFuseResponseMessages.version    = "
              << doFuseResponseMessages.version << std::endl;
    std::cout << "doFuseResponseMessages.algorithms = "
              << doFuseResponseMessages.algorithms << std::endl;

    if (doFuseResponseMessages.version > 0)
    {
        config.fuseThrRespMessages.version =
            WrapperConfig::proc2thr(doFuseResponseMessages.version);
    }

    if (doFuseResponseMessages.capability > 0)
    {
        config.fuseThrRespMessages.capability =
            WrapperConfig::proc2thr(doFuseResponseMessages.capability);
    }

    if (doFuseResponseMessages.algorithms > 0)
    {
        config.fuseThrRespMessages.algorithms =
            WrapperConfig::proc2thr(doFuseResponseMessages.algorithms);
    }

    if (doFuseResponseMessages.digests > 0)
    {
        config.fuseThrRespMessages.digests =
            WrapperConfig::proc2thr(doFuseResponseMessages.digests);
    }
    ...
    {
        config.fuseThrRespMessages.measurements =
            WrapperConfig::proc2thr(doFuseResponseMessages.measurements);
    }
    if (respMearurement.nonce > 0)
    {
        config.fuseRespMearurement.nonce =
            WrapperConfig::proc2thr(respMearurement.nonce);
    }
    if (respMearurement.measurementBlock > 0)
    {
        config.fuseRespMearurement.measurementBlock =
            WrapperConfig::proc2thr(respMearurement.measurementBlock);
    }
    if (respMearurement.measurementBlockLen > 0)
    {
        config.fuseRespMearurement.measurementBlockLen =
            WrapperConfig::proc2thr(respMearurement.measurementBlockLen);
    }
    if (respMearurement.measurementBlockVal > 0)
    {
        config.fuseRespMearurement.measurementBlockVal =
            WrapperConfig::proc2thr(respMearurement.measurementBlockVal);
    }
    if (respMearurement.opaqueData > 0)
    {
        config.fuseRespMearurement.opaqueData =
            WrapperConfig::proc2thr(respMearurement.opaqueData);
    }
    if (respMearurement.opaqueDataLen > 0)
    {
        config.fuseRespMearurement.opaqueDataLen =
            WrapperConfig::proc2thr(respMearurement.opaqueDataLen);
    }
    if (respMearurement.opaqueDataVal > 0)
    {
        config.fuseRespMearurement.opaqueDataVal =
            WrapperConfig::proc2thr(respMearurement.opaqueDataVal);
    }
    if (respMearurement.signature > 0)
    {
        config.fuseRespMearurement.signature =
            WrapperConfig::proc2thr(respMearurement.signature);
    }
    if (respMearurement.signatureLen > 0)
    {
        config.fuseRespMearurement.signatureLen =
            WrapperConfig::proc2thr(respMearurement.signatureLen);
    }
    if (respMearurement.signatureVal > 0)
    {
        config.fuseRespMearurement.signatureVal =
            WrapperConfig::proc2thr(respMearurement.signatureVal);
    }

    config.alterHeaderThr.all = WrapperConfig::proc2thr(alterHeaderProb.all);
    config.alterHeaderThr.version =
        WrapperConfig::proc2thr(alterHeaderProb.version);
    config.alterHeaderThr.messageType =
        WrapperConfig::proc2thr(alterHeaderProb.messageType);
    config.alterHeaderThr.param =
        WrapperConfig::proc2thr(alterHeaderProb.param);
    config.alterDataThr = WrapperConfig::proc2thr(alterDataProb);
    cout << "Current config:" << std::endl;
    cout << config << std::endl;

    cout << "config.fuseThrRespMessages.version = "
         << config.fuseThrRespMessages.version << std::endl;
}
