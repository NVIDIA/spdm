...
namespace spdmcpp
{
	class EMU_MCTP_TransportClass : public TransportClass	//for matching dmtf spdm emulator --trans MCTP
	{
	public:
	...
	class MCTP_TransportClass : public TransportClass	//for sending to mctp-demux-daemon
	{
	public:
		...
		virtual RetStat decode(std::vector<uint8_t>& buf, LayerState& lay)//TODO is any of this correct at all?! can't test and verify atm
		{
			set_layer_size(lay, sizeof(header_type));
			auto& header = get_header_ref<header_type>(buf, lay);
...
public:
	...
	~EMUIOClass() override;
	spdmcpp::RetStat write(const std::vector<uint8_t>& buf, spdmcpp::timeout_us_t timeout = spdmcpp::TIMEOUT_US_INFINITE) override;
	spdmcpp::RetStat read(std::vector<uint8_t>& buf, spdmcpp::timeout_us_t timeout = spdmcpp::TIMEOUT_US_INFINITE) override;
	spdmcpp::RetStat setup_timeout(spdmcpp::timeout_us_t timeout = spdmcpp::TIMEOUT_US_INFINITE) override;
private:
	EmulatorBase& Emulator;
};

class DemuxIOClass : public spdmcpp::IOClass	//TODO decouple from EmulatorBase and generalize
{
public:
...
public:
	...
	EmulatorBase(int socket) : Socket(socket), Event(sdeventplus::Event::get_default())
	{
	}
	~EmulatorBase()
	{
	...
	//TODO rename for clearer layer separation and meaning ! and depend more on function type overloading?
	...
	bool write_bytes(const uint8_t* buf, size_t size)
	{
		...
		while (sent < size) {
			...
			if (ret == -1) {
				std::cerr << "EmulatorBase::write_bytes() errno = " << errno << std::endl;
				return false;
			}
			sent += ret;
		}
	...
	bool read_bytes(uint8_t *buf, size_t size)
	{
		...
		while (done < size) {
			...
			if (result == -1) {
				std::cerr << "EmulatorBase::read_bytes() errno = " << errno << std::endl;
				return false;
			}
			if (result == 0) {
				return false;
	...
	bool send_platform_data(SocketCommandEnum command, const std::vector<uint8_t>& buf)
	{
	...
	bool receive_platform_data(SocketCommandEnum &command, std::vector<uint8_t>& recv)
	{
		...
		}
		{
			...
			assert(static_cast<SocketTransportTypeEnum>(transport_type) == TransportType);
		}
		if (!receive_buf(recv)) {
			return false;
		}
	...
	bool send_message_receive_response(SocketCommandEnum command,
					const buffer_t& send,
					SocketCommandEnum &response, buffer_t& recv)
	{
